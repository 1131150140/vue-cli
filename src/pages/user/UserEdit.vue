     ...
     <el-form :model="ruleForm"  ref="ruleForm" label-width="100px" class="demo-ruleForm">
       <el-row :gutter="12">
         ...
         <el-form-item label="用户号码" >
          <el-col :span="10">
            <el-input size="small" readonly class="shesi" v-model="ruleForm.mobile" placeholder="请填写用户手机号作为用户登录账号"></el-input>
          </el-col>
        </el-form-item>
...
<script>
import {addCompany, getqiyexinxi, getcompanystructure, userroleAdd, getrolesbydepart, useradd, getbreifui, edituibyadmin} from '@/api/index'
export default {
   data() {
     return {
        ruleForm: {
     ...
     methods: {
      ...
      submitForm() {
        ...
        edituibyadmin(this.ruleForm).then(res => {
          if (res.code == 200) {
               this.open3('成功修改用户信息!');
          } else {
      ...
      getgerenxinxi() {
       ...
       getbreifui(userId).then(res => {
         console.log(res)
         this.companyId = res.data.companyid;
         this.companyName = res.data.departName;
         this.ruleForm.realname = res.data.realname;
         this.ruleForm.roleid = res.data.roleid;
         this.ruleForm.mobile = res.data.mobile;
         this.ruleForm.departid = res.data.departid;
         this.dataScope = JSON.parse(res.data.dataScope)

...
<div class="loanList">
    ...
    <el-col :span="24">
        <div class="userManager">
            <div class="jiegou"><i class="el-icon-d-caret"></i>银行管理   
            <el-button @click= "xinzengyonghu" class="youfu" size="small" type="primary">新增产品</el-button></div>     
        </div>
...
<template slot-scope="scope">
   <el-button @click="handleDelete(scope.row.id,scope.row.name, scope.$index)" type="text" >删除</el-button>
    <router-link
            :to="{ path:'/loanEdit', query: {'id': scope.row.id} }"
            tag="span">
...
<script>
import {financialSchemeList, financialSchemeDel} from '@/api/index'
...
import Paging from '@/common/Paging'
  export default {
    ...
    getUserAll(str) {
        financialSchemeList(str).then(res => {
            res.data.list.forEach(element => {
                element.updateDate = formaTime(element.updateDate)
            });
            this.tableData = res.data.list
